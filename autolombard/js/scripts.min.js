...
$(document).ready(function () {
  ...
  var carsSlider = new Swiper('.js-cars-slider', {
    slidesPerView: 1,
    centeredSlides: true,
    initialSlide: 1,
    navigation: {
  ...
  var salesSlider = new Swiper('.js-sales-slider', {
    slidesPerView: 1,
    spaceBetween: 40,
    simulateTouch: true,
    navigation: {
  ...
  var swiper, conditions, steps, advantages, documents;

  function resizeScrenn() {
    if ($(window).width() < 769 && swiper == undefined || advantages != undefined) {
      console.log($(window).width());

      if ($(".js-mob-slider").length > 0) {
        swiper = new Swiper('.js-info-slider', {
          slidesPerView: 1,
          spaceBetween: 0,
          pagination: {
            el: '.swiper-pagination',
        ...
        conditions = new Swiper('.js-conditions-slider', {
          slidesPerView: 3,
          spaceBetween: 0,
          simulateTouch: true,
          pagination: {
        ...
        steps = new Swiper('.js-steps-slider', {
          slidesPerView: 1,
          spaceBetween: 20,
          simulateTouch: true,
          pagination: {
        ...
        advantages = new Swiper('.js-advantages-slider', {
          slidesPerView: 2,
          spaceBetween: 20,
          simulateTouch: true,
          pagination: {
        ...
        documents = new Swiper('.js-documents-slider', {
          slidesPerView: 2,
          simulateTouch: true,
          pagination: {
            el: '.swiper-pagination',
    ...
    } else if ($(window).width() > 768 && swiper != undefined || advantages != undefined) {
      swiper && swiper.destroy();
      conditions && conditions.destroy();
      steps && steps.destroy();
      advantages && advantages.destroy();
    ...
    pips: {
      ...
      format: {
        to: function to(value) {
          return value == 0 ? "".concat(value, " &#8381;") : "".concat(value / 1000000, " \u043C\u043B\u043D &#8381;");
        },
        thousand: ' ',
        from: Number
      }
  ...
  range && range.noUiSlider.on('update', function (value, handle) {
    var currentVal = parseInt(range.noUiSlider.get()).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    var price = $('.js-range-price');
    price.text("".concat(currentVal, " \u20BD"));
    calculateInfo(range.noUiSlider.get());
  });
  $('.js-calculator-card').on('click', function () {
    var activeClass = 'b-calculator__item--active';
    $('.js-calculator-card').removeClass(activeClass);
    $(this).addClass(activeClass);
    var percent = $(this).find('.js-percent').text();
    $('.js-current-percent').text(percent);
    var rangePrice = parseInt($('.js-range-price').text().replace(/\s/g, '').slice(0, -1));
    calculateInfo(rangePrice);
  });
  $('.b-faq__item').on('click', function (e) {
    $(this).toggleClass('b-faq__item--active');
    var answer = $(this).find('.b-faq__answer');
    answer.slideToggle();
  });
  /** Popup **/
  ...
  var popup = $('.b-popup'),
      ...
      popupPayment = $('.b-popup--payment'),
      popupReviews = $('.b-popup--reviews'),
      popupSended = $('.b-popup--sended'),
  ...
  $('.js-show-popup--send').on('click', function (e) {
    e.preventDefault();
    showPopup(popupSend);
  });
  $('.js-show-popup--thanks').on('click', function (e) {
    e.preventDefault();
    closePopup();
    showPopup(popupThanks);
  });
  $('.js-show-popup--payment').on('click', function (e) {
    e.preventDefault();
    showPopup(popupPayment);
  });
  $('.js-show-popup--reviews').on('click', function (e) {
    e.preventDefault();
    showPopup(popupReviews);
  });
  $('.js-show-popup--sended').on('click', function (e) {
    e.preventDefault();
    closePopup();
    showPopup(popupSended);
  });
  ...
  var drag = $(".js-drag"),
      input = drag.find('input'),
      maxFileSize = 4000000; // 4мб
  ...
  if (isAdvancedUpload) {
    ...
    drag.on('drag dragstart dragend dragover dragenter dragleave drop', function (e) {
      ...
      drag.addClass('b-form__drag--error');
    }).on('dragleave dragend drop', function () {
      drag.removeClass('b-form__drag--error');
    }).on('drop', function (e) {
      $('.b-calculator__images').empty();
      droppedFiles = e.originalEvent.dataTransfer.files;
      input.prop('files', droppedFiles);
      console.log(droppedFiles);

      for (var i = 0; i < droppedFiles.length; i++) {
        $('.b-calculator__images').append("<span class=\"b-calculator__img-name \">".concat(droppedFiles[i].name, "</span>"));
      }
    });
  }
  /** Mobile burger **/
  ...
  $('.b-burger').on('click', function () {
    ...
    $('.b-submenu').hide();
    $('.b-menu__item--down').removeClass('b-menu__item--active');
  });
  $('.b-menu__item').not('.b-menu__item--down').on('click', function (e) {
  ...
  $('.b-submenu__item').not('.b-submenu__item--title').on('click', function (e) {
    e.stopImmediatePropagation();
    console.log(true);
    $('.b-burger').removeClass('b-burger--active');
    $('.b-nav').removeClass('b-nav--active');
  ...
  $('.b-submenu__item--title').on('click', function (e) {
    e.preventDefault();
    e.stopImmediatePropagation();
    $('.b-submenu').hide();
    $('.b-menu__item--down').removeClass('b-menu__item--active');
  });
  $('.b-menu__item--down').on('click', function (e) {
    e.preventDefault();
    e.stopImmediatePropagation();
    console.log(e.target);
    $(this).addClass('b-menu__item--active');
    $(this).find('.b-submenu').fadeIn('fast');
  });
  /** Toogle Calc **/

  function curretItem(toggler) {
    var data = toggler.data('calc');
    var item = $('.b-calculator__item'),
        itemClass = "b-calculator__item--".concat(data);
    item.removeClass('b-calculator__item--active');
    $(".".concat(itemClass)).addClass('b-calculator__item--active');
    var percent = $(".".concat(itemClass)).find('.js-percent').text();
    $('.js-current-percent').text(percent);
    var rangePrice = parseInt($('.js-range-price').text().replace(/\s/g, '').slice(0, -1));
    calculateInfo(rangePrice);
  }

  $('.b-calculator__toggle').on('click', function (e) {
    e.stopImmediatePropagation();
    curretItem($(this));
    var activeClass = 'b-calculator__toggle--active';
    $('.b-calculator__toggle').removeClass(activeClass);
    $(this).addClass(activeClass);
    console.log(parseInt($('.js-range-price').text()));
  }); // Calc

  function calculateInfo(rangeVal) {
    var minSum, monthPay;

    if ($('.b-calculator__item--active').hasClass('b-calculator__item--auto')) {
      minSum = Math.floor(rangeVal * 100 / 95).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
      monthPay = Math.floor(rangeVal * 0.03).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    } else {
      minSum = Math.floor(rangeVal * 100 / 85).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
      monthPay = Math.floor(rangeVal * 0.04).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    }

    $('.js-current-sum').text("\u0434\u043E ".concat(minSum, " \u20BD"));
    $('.js-current-price').text("".concat(monthPay, " \u20BD"));
  }
});